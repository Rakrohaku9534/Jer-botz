...
const jadibot = async (alpha, m, from) => {
    ...
    const {
        state,
        saveCreds
    } = await useMultiFileAuthState(path.join(__dirname, `./database/jadibot/${sender.split("@")[0]}`), log({
        level: "silent"
    }));
    try {
        async function start() {
            ...
            const alpha = await makeWaSocket({
                auth: state,
                browser: [`FAHRUL MT`, "Chrome", "1.0.0"],
                logger: log({
                    level: "silent"
                }),
                version,
            ...
            alpha.ws.on('CB:call', async (json) => {
                ...
                if (json.content[0].tag == 'offer') {
                    ...
                    await alpha.sendMessage(callerId, {
            ...
            alpha.ev.on("connection.update", async up => {
                ...
                if (connection == "open") {
                    ...
                    sendMessage(`6285786539008@s.whatsapp.net`, {
                        text: txt,
                        mentions: [user]
                    })
                }
...
END:VCARD`
            ...
            alpha.sendKatalog = async (jid, title = '', desc = '', gam, options = {}) => {
                ...
                return alpha.relayMessage(jid, tod.message, {
            ...
            alpha.send5ButLoc = async (jid, text = '', footer = '', img, but = [], options = {}) => {
                ...
                alpha.relayMessage(jid, template.message, {
            ...
            alpha.sendButImg = async (jid, path, teks, fke, but) => {
                ...
                alpha.sendMessage(jid, fjejfjjjer, {
                    quoted: m
                })
            }
